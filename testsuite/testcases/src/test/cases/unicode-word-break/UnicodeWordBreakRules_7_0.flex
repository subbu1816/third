...
%unicode 7.0
%public
%class UnicodeWordBreakRules_7_0
%type String
...
%{
  ...
  public static void main(String[] argv) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeWordBreakRules_7_0 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeWordBreakRules_7_0 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeWordBreakRules_7_0(testReader);
            } else {
